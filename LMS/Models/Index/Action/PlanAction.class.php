...
class PlanAction extends CommonAction
{
    ...
    public function mission_complete(){
        ...
        //之所以要讲time放到$supervision_log中，是因为在计划监控中需要时间，但也不想再从这边取，然后我就把mission_complete中的time删了，，后悔已来不及啊啊啊啊啊啊啊！！！！
    ...
    public function delete(){
        ...
        //找到计划
        $plan_clone=M('plan_clone')->field('svid,uid')->find($pcid);
        //错误处理
        if(empty($plan_clone))
            $this->error('没有该计划');
    ...
    public function addPlanHandle(){
        ...
        //这里最好不报错，进行容错处理，因为报错返回会导致计划表单填写全部丢失！
