...
class SupervisionAction extends CommonAction {
    ...
    public function waiting(){
        ...
        $data=$db
            ...
            ->where("pc.svid='%d' AND pc.uid IS NOT NULL",$uid)
            ->field('sv.id,sv.pcid,sv.status,sv.info,sv.reply,sv.star,sv.title,sv.complete_time,pc.uid,plan.name,plan.face')
            ->order('sv.status ASC,sv.id DESC')
    ...
    public function agree(){
        ...
        if(M('plan_clone')->save($arr)){
            ...
            $data['text']='监督状态没有任何修改，计划已不存在或者您已监督此计划。';
        }
        $this->ajaxReturn($data);
    }
    public function refuse(){
        ...
        //验证plan_clone是否已被删除。
        if(empty($plan_clone['id'])) {
            $data['text'] = '该计划已不存在';
            $data['status'] = $db->delete($req_id);
            $this->ajaxReturn($data);
        }
        if($req['status']){
