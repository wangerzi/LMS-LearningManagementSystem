$(function(){
	...
	$("form").submit(function(){
		...
		if(password.val().length<minPas||password.val().length>maxPas) {
			inputError(password_2, $("#login-area .error-info"), "密码不得小于" + minName + "位，不得多于" + maxPas + "位");
			return false;
		}

		//验证邮箱格式！
		if(/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(email.val())!=true) {
